...
- `ParseUUIDPipe`: Valida que el string sea un UUID (ej. `123e4567-e89b-12d3-a456-426614174000`).
...
3. **Pipes:** El ID de las tareas debe ser un identificador único global (**UUID v4**). Por lo tanto, utiliza el decorador nativo `ParseUUIDPipe` para validar el ID en todas las rutas requeridas.
4. **Estado en Memoria:** Las tareas se almacenarán en un arreglo estático dentro del servicio. La interfaz `Task` debe tener la forma: `{ id: string, title: string, description: string, isCompleted: boolean }`.
