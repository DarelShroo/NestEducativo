...
class CodeEditor {
    constructor(textareaId) {
        ...
        const nestjsKeywords = [
            'Injectable', 'Controller', 'Get', 'Post', 'Patch', 'Delete', 'Put',
            'Param', 'Body', 'Query', 'Headers', 'Req', 'Res', 'Module',
            'NotFoundException', 'BadRequestException', 'UnauthorizedException',
            'ParseIntPipe', 'ParseUUIDPipe', 'PipeTransform', '@nestjs/common',
            'ExecutionContext', 'CallHandler', 'NestInterceptor', 'CanActivate',
            'UseGuards', 'UseInterceptors', 'UsePipes', 'SetMetadata',
            'createParamDecorator', 'applyDecorators'
        ];
        ...
        CodeMirror.registerHelper("hint", "nestjs", function(cm) {
            ...
            let start = token.start, end = token.end, word = token.string;
            if (word === '@') { word = ''; }
    ...
    openTab(filePath, content) {
        ...
        const fileContent = content != null ? content : (window.vfs ? window.vfs.getContent(filePath) || '' : '');
    ...
    _startRenameTab(index, nameSpan) {
        ...
        input.addEventListener('keydown', (e) => {
            ...
            if (e.key === 'Escape') { input.value = oldBasename; input.blur(); }
        });
    }
    ...
    getValue() {
        ...
        return Object.entries(files).map(([p, content]) => `// ---- ${p} ----\n${content}`).join('\n\n');
    }
