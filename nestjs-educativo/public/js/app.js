...
function initializeApp() {
    ...
    if (savedUIState && savedUIState.currentLevel) {
        handleLevelChange(savedUIState.currentLevel).then(() => {
            ...
            } else if (savedUIState.currentSection === 'ejercicio-detalle' && savedUIState.currentExerciseId) {
    ...
    if (window.NestCLI) {
        ...
        nestCli.onOutput = (msg, isError) => {
            ...
            line.className = isError ? 'error' : (msg.startsWith('CREATE') || msg.startsWith('UPDATE') ? 'success' : '');
...
function attachEventListeners() {
    ...
    if (elements.openHttpSimulatorBtn) elements.openHttpSimulatorBtn.addEventListener('click', () => {
        elements.httpSimulatorModal.classList.remove('hidden');
    });
    if (elements.closeHttpModalBtn) elements.closeHttpModalBtn.addEventListener('click', () => {
...
function showSection(section) {
    ...
    elements.progresoSidebar.classList.add('hidden');
    if (elements.floatingToggleProgresoBtn) elements.floatingToggleProgresoBtn.classList.add('hidden');
...
function handleShowSolution() {
    ...
    if (confirm(confirmMsg)) {
        if (solutionFiles && window.codeEditor) {
            window.codeEditor.loadProject(JSON.parse(JSON.stringify(solutionFiles)));
...
async function handleValidation() {
    ...
    const files = window.codeEditor ? window.codeEditor.getProjectFiles() : { 'src/main.ts': elements.codeEditor.value };
...
async function handleHttpSimulation() {
    ...
    try {
        ...
        if (result.success && result.response) {
            ...
            const resData = typeof result.response.data === 'object' ? 
