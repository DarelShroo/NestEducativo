...
class LanguageService {
    ...
    static getThisSuggestions(content, searchWord) {
        ...
        if (constructorMatch) {
            ...
            while ((pMatch = paramRegex.exec(paramsStr)) !== null) {
                suggestions.push({ text: pMatch[1], displayText: pMatch[1] + " (Service)", type: 'property', refType: pMatch[2] });
            }
        }
        
        return searchWord ? suggestions.filter(s => s.text.toLowerCase().startsWith(searchWord.toLowerCase())) : suggestions;
    }
    ...
    static getServiceSuggestions(currentContent, serviceVarName, searchWord) {
        ...
        const paramRegex = new RegExp(`(?:private|protected|public)?\\s*(?:readonly)?\\s*${serviceVarName}\\s*:\\s*([a-zA-Z0-9_]+)`);
