...
class NestCLI {
    constructor() {
        this.aliases = { mo: 'module', co: 'controller', s: 'service', res: 'resource', gu: 'guard', in: 'interceptor', pi: 'pipe', d: 'decorator', f: 'filter' };
        this.history = [];
        this.onOutput = null;
    }
    ...
    execute(input) {
        ...
        if (parts[0] !== 'nest') return this._out(`Error: comando desconocido '${parts[0]}'. Usa 'nest'.`, true);
    ...
    _generate(schematic, name) {
        ...
        if (!gen) return this._out(`Schematic desconocido '${schematic}'. Tipos: module, controller, service, resource, guard, interceptor, pipe, decorator, filter`, true);
        return gen();
    }
    ...
    _genGuard(name) {
        ...
        this._createFiles([{ path: `src/common/guards/${name}.guard.ts`, content }], `CREATE src/common/guards/${name}.guard.ts`);
    }
    ...
    _genInterceptor(name) {
        ...
        this._createFiles([{ path: `src/common/interceptors/${name}.interceptor.ts`, content }], `CREATE src/common/interceptors/${name}.interceptor.ts`);
    }
    ...
    _genPipe(name) {
        ...
        this._createFiles([{ path: `src/common/pipes/${name}.pipe.ts`, content }], `CREATE src/common/pipes/${name}.pipe.ts`);
    }
    ...
    _genDecorator(name) {
        ...
        this._createFiles([{ path: `src/common/decorators/${name}.decorator.ts`, content }], `CREATE src/common/decorators/${name}.decorator.ts`);
    }
    ...
    _genFilter(name) {
        ...
        this._createFiles([{ path: `src/common/filters/${name}.filter.ts`, content }], `CREATE src/common/filters/${name}.filter.ts`);
    }
